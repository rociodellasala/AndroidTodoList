...
class ListActivity : AppCompatActivity(), SelectedDialogItems, OnListClickedListener, ListView {
    ...
    private fun setExtendedFloatingButtonAction() {
        ...
        addListFAB.setOnClickListener { view: View? -> presenter!!.onButtonClicked() }
    }
    ...
    override fun showFilterDialog() {
        ...
        val filterDialog: FilterDialogFragment = FilterDialogFragment.Companion.newInstance(adapter!!.categories, adapter.getFilterSelections())
    ...
    override fun showSortByDialog() {
        ...
        val sortByDialog: SortByDialogFragment = SortByDialogFragment.Companion.newInstance(Objects.requireNonNull(adapter).getSortIndex())
    ...
    override fun onSelectedItems(klass: Class<*>, items: MutableList<Int>?) {
        if (klass == SortByDialogFragment::class.java) adapter.setSortIndex(items!![0]) else adapter.setFilterSelections(items)
    }
